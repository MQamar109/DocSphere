...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            ...
            self.stdout.write(f"{self.get_organization_data(count)}")
        else:
            Organization.objects.bulk_create(self.get_organization_data(count))
            self.stdout.write(self.style.SUCCESS(f"{count} records has been updated in organization"))

    def get_organization_data(self, count):
        return [Organization(name = f"organization #{i}", description= f"description #{i}", is_active= bool(i%2)) for i in range(count)]